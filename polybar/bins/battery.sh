...
if [[ $CHARGE = *"Charging"* ]]; then
    ...
    ICON_CHRG=""
    MSG=$CHARGE
elif [[ $CHARGE = *"Full"* || $CHARGE = *"Not"* ]]; then
    active="-u 1"
    ICON_CHRG=""
    MSG=$CHARGE
else
...
if [ "$CHARGE" = "Full" ]; then
    ...
    LIFE="$(acpi | cut -d, -f3 | grep -oP '\d{2}:\d{2}')"
fi


## Main
chosen="$(echo -e "$options" | $rofi_command -p "$BATTERY%, $LIFE left" -dmenu $active $urgent -selected-row 0)"
case $chosen in
    $ICON_CHRG)
        ;;
    $ICON_DISCHRG)
